...
class Approval extends React.Component {
  ...
  constructor(props) {
    ...
    this.columns = [{
      ...
      render: (text, record) => (
        <span>
          <a onClick={() => {this.openTagApproval(record)}}>审批</a>
        </span>
      )
    }]
  }
  ...
  openTagApproval = (record) => {
    ...
    this.props.dispatch({
      ...
      payload: { record, page: 'Approval' },
    })

    if(record.applyType !== 3) {
      this.props.dispatch({
        type: 'approval/queryDevUser',
      })
    }
  }
  ...
  render() {
    ...
    return ( 
      <div className={styles.Approval}>
        <PageHeaderLayout breadcrumbList={[{ title: '首页', href: '/' }, { title: '审批管理' }]} />
